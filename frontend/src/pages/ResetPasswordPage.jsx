...
// /reset-password?token=... (see backend/app/notifications.py's
// APP_BASE_URL + reset_link construction).
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!token ? (
          <div style={{ textAlign: 'center', fontSize: 13, color: 'var(--tier-red)' }}>
            This link is missing its reset token. Please use the link from your
            email, or <Link to="/forgot-password" style={{ color: 'var(--color-teal)' }}>request a new one</Link>.
          </div>
        ) : success ? (
